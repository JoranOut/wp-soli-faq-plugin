...
/**
 * Hides FAQs from not-logged-in visitors on every surface:
 *
...
class Visibility {
	...
	/**
	 * Empty every logged-out query that targets FAQs, or strip the
	...
	/**
	 * 403 for logged-out visits to a single faq or any main query that
	 * targets the post type (e.g. ?post_type=soli_faq), following the
	...
	public function guard_front_end() {
		...
		wp_die(
			esc_html__( 'FAQs are only visible to members. Please log in to view them.', 'soli-faq' ),
			esc_html__( 'Forbidden', 'soli-faq' ),
			array( 'response' => 403 )
		);
	}

	/**
	 * 403 for logged-out REST requests to the FAQ routes. Logged-in
	...
	public function guard_rest( $response, $handler, $request ) {
		...
		return new \WP_Error(
			'rest_forbidden',
			__( 'FAQs are only visible to members.', 'soli-faq' ),
			array( 'status' => 403 )
		);
	}
}
